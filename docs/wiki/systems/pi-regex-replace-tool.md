...
- `extensions/regex-replace.ts` owns Pi schema, approval metadata, model-call guidance, rendering, and bounded model output. Native Pi result details retain only `dryRun`, `totalReplacements`, and `filesModified` plus a bounded `diff`, keeping serialized details below Pi's 10 KiB runtime cap; oversized diffs are explicitly marked as truncated instead of becoming `Invalid replacement details`.
